...
@Getter
// 엔티티 관계설정에서 엔티티의 toString()이 호출되면 또다른 엔티티의 toString()이 호출되기 때문에 엔티티 간의 toString() 호출로 stackoverflow 에러 발생
// 엔티티 어느 한 쪽에서는 toString() 호출을 제외(exclude)에서 순환 참조 호출 방지
